#!/usr/bin/env python3
"""
Generic export: orders for any product to any customer.
Cost matched to order date from MiSys MIICST + MIPOD.
FX: Bank of Canada historical USD/CAD rates (fallback: Sage dExchRate, then USD_CAD_RATE).
"""
import csv
import json
import os
...
def _load_boc_usdcad_rates(start_date, end_date):
    """
    Fetch Bank of Canada USD/CAD FX rates for date range.
    Returns (rates_dict, sorted_dates) where rates_dict[date_str] = rate.
    For weekends/holidays, use nearest prior business day.
    """
...
def _build_item_variants(item_search):
    """Build search variants from item_search (e.g. REOLUBE46B -> reolube46b, reol46b, etc)."""
    item_lower = (item_search or "").lower().strip()
    if not item_lower:
        return []
    item_parts = re.sub(r"[^a-z0-9]", "", item_lower)
    variants = [item_lower, item_parts]
    _ALIASES = [
        (r"reolube\s*", "reol"),
        (r"reolibe\s*", "reol"),
        (r"reolue\s*", "reol"),
    ...
    for pat, repl in _ALIASES:
        if re.search(pat, item_lower):
            ...
            variants.append(re.sub(r"[^a-z0-9]", "", normalized))
            break
...
def _load_misys_cost_history():
    ...
    for r in mipod:
        ...
        if cost <= 0:
            continue
        # MIPOD may store unit cost OR extended (line total). If we have qty and cost/qty is a
        # reasonable drum price (2k–8k USD), treat cost as extended and use cost/qty.
...
def _safe_filename(s):
    return re.sub(r'[^\w\s\-]', '', str(s or "").strip()).replace(" ", "_")[:40] or "export"

def export_customer_item_orders(customer_search: str, item_search: str, out_dir: str = None) -> tuple:
    """
    Export orders for any product to any customer to Excel (pricing, cost, profit, margin).
    FX: Bank of Canada historical USD/CAD rates (fallback: Sage dExchRate, then USD_CAD_RATE).
    Returns (file_path, None) on success or (None, error_message) on failure.
    """
    cust_search = (customer_search or "").strip()
    item_search_str = (item_search or "").strip()
    if not cust_search or not item_search_str:
        return None, "customer_search and item_search are required"

    if not os.path.isdir(SAGE_BASE):
        return None, f"Sage folder not found: {SAGE_BASE}"
    subfolders = [f for f in os.listdir(SAGE_BASE)
                 if os.path.isdir(os.path.join(SAGE_BASE, f)) and not f.startswith("_")]
    if not subfolders:
        return None, "No Sage subfolders found"
    ...
    if not tsalordr or not tsoline or not tinvent:
        return None, "Missing Sage data (tsalordr, tsoline, tinvent)"

    cost_history, current_usd, current_cad = _load_misys_cost_history()

    cust_lower = cust_search.lower()
    item_variants = _build_item_variants(item_search_str)
    ...
    if not cust_ids:
        return None, f"No customers found matching '{cust_search}'"
    ...
    for r in tsalordr:
        ...
        if cid in cust_ids:
            ...
            if dt_str:
                order_dates_for_fx.append(_parse_date(dt_str))

    boc_rates, boc_sorted_dates = {}, []
    if order_dates_for_fx:
        valid = [d for d in order_dates_for_fx if d is not None]
        if valid:
            boc_rates, boc_sorted_dates = _load_boc_usdcad_rates(min(valid), max(valid))
    ...
    for row in tsoline:
        ...
        margin_pct_usd = round((profit_usd / rev_usd * 100), 1) if rev_usd > 0 and profit_usd is not None else None
        margin_pct_cad = round((profit_cad / rev_cad * 100), 1) if rev_cad > 0 and profit_cad is not None else None
        records.append({
            ...
            "margin_pct_usd": margin_pct_usd,
            "margin_pct_cad": margin_pct_cad,
        })

    if not records:
        return None, f"No orders found for '{item_search_str}' to '{cust_search}'"

    records.sort(key=lambda x: x["order_date"], reverse=True)

    title_item = (item_search_str or "Product").strip()
    title_cust = (cust_search or "Customer").strip()
    ...
    def _write_sheet(ws, title, curr, rev_col, cost_col, tc_col, profit_col, margin_pct_col):
        ...
        ws["A1"] = f"{title_item} {title_cust} — ALL IN {curr}"
    ...
    ws_usd = wb.active
    _write_sheet(ws_usd, "USD", "USD", "rev_usd", "cost_usd", "total_cost_usd", "profit_usd", "margin_pct_usd")
    ws_cad = wb.create_sheet("CAD")
    summary_row = _write_sheet(ws_cad, "CAD", "CAD", "rev_cad", "cost_cad", "total_cost_cad", "profit_cad", "margin_pct_cad")
    ...
    chart_data_first_row = chart_start - data_rows
    if data_rows > 0:
        chart_usd = BarChart()
        chart_usd.type = "col"
        chart_usd.title = f"{title_item} {title_cust} - Revenue vs Cost (USD)"
        ...
        chart_cad = BarChart()
        chart_cad.type = "col"
        chart_cad.title = f"{title_item} {title_cust} - Revenue vs Cost (CAD)"
    ...
    base_name = f"{_safe_filename(title_item)}_{_safe_filename(title_cust)}_Orders.xlsx"
    out_path = os.path.join(out_dir, base_name)
    try:
        wb.save(out_path)
    except PermissionError:
        alt = os.path.join(out_dir, base_name.replace(".xlsx", "_NEW.xlsx"))
        wb.save(alt)
        out_path = alt
