...
_FOCUSED_PROMPTS: Dict[str, str] = {
    ...
    "pricing": (
        ...
        "- Find the best matching product row(s) — be smart about partial name matches\n"
        "  (e.g. 'MOv Long Life 1' should match rows containing those keywords)\n"
        "- Show: Product Name / Description, Container/Size, Price, Currency, Price List Tier\n"
        "- If a customer name was provided (e.g. 'for Georgia Western'), look for customer-specific "
        "  pricing rows first; fall back to standard pricing if none found\n"
        "- Present the price clearly: e.g. '$45.00 CAD per drum (Regular price list)'\n"
        "- If multiple matching products are found, list all of them\n"
        "- Do NOT fabricate any prices — only use values from the DATA section\n"
        "- If the product is not found, say so clearly and list the closest matches"
    ),
}
