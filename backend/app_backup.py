...
def get_all_data():
    ...
    try:
        print("/api/data endpoint called")
            so_data['tax'] = safe_float(tax_match.group(1))
        
        # Extract total amount from "Total Amount" line
        total_patterns = [
            r'Total Amount\s+(?:US\$|CAD\$|\$)?([\d,]+\.?\d*)',  # Canoil format with currency
            r'Total Amount\s+([\d,]+\.?\d*)',  # Without currency
            r'Total:\s*\$?([\d,]+\.?\d*)',
            r'Grand Total:\s*\$?([\d,]+\.?\d*)'
        ]
        ...
        for pattern in total_patterns:
            match = re.search(pattern, full_text, re.IGNORECASE)
            if match:
                so_data['total_amount'] = safe_float(match.group(1))
                break
        
        # If no total found, use subtotal + tax
        if so_data['total_amount'] == 0.0 and so_data['subtotal'] > 0:
            so_data['total_amount'] = so_data['subtotal'] + so_data['tax']
        
        # Extract items (enhanced for Canoil format)
        items = []
        ...
        # Preprocess lines to handle split items
        processed_lines = []
        i = 0
        while i < len(lines):
            line = lines[i].strip()
            ...
            if 'Item No.' in line and 'Ordered' in line and 'Description' in line:
                in_item_section = True
                i += 1
                continue
            
            # End of item section
            if in_item_section and ('EMAIL' in line or 'E-mail' in line or 'SDS:' in line or 'Subtotal:' in line or 'Shipped By:' in line or 'Terms:' in line):
                break
            
            if in_item_section and line and len(line) > 5:
                # Check if next line might be a continuation (like "CASE CASE")
                if i + 1 < len(lines):
                    next_line = lines[i + 1].strip()
                    # If next line is short and looks like a continuation, merge it
                    if len(next_line) < 20 and next_line and not any(keyword in next_line for keyword in ['MOVLL', 'Pallet', 'Brokerage', 'Freight']):
                        line = line + ' ' + next_line
        ...
        for line in processed_lines:
                # Skip lines that are clearly not items (but allow Freight Charge as an item)
                if any(keyword in line.upper() for keyword in ['EMAIL', 'INVOICE', 'BOL', 'ESTES', 'ACCOUNT']) and 'CHARGE' not in line.upper():
                    break  # These mark the end of items
                
                # Parse different line formats
                item_match = None
                
                # Handle merged lines with multiple data points
                # Generic pattern: ITEMCODE QUANTITY UNIT DESCRIPTION... PRICE US$TOTAL
                merged_match = re.match(r'^([A-Z0-9]+)\s+(\d+)\s+([A-Z0-9]+)\s+(.*?)\s+([\d,]+\.?\d*)\s+US\$([\d,]+\.?\d*)(?:\s+.*)?$', line)
                if merged_match:
                    item_code = merged_match.group(1)
                    quantity = int(merged_match.group(2))
                    unit = merged_match.group(3)
                    raw_description = merged_match.group(4)
                    unit_price = safe_float(merged_match.group(5))
                    total_price = safe_float(merged_match.group(6))
                    
                    # Extract clean product name from SO description
                    # Look for main product name pattern and clean it
                    description_words = []
                    words = raw_description.split()
                    
                    # Find the core product name (usually starts with brand/product name)
                    for i, word in enumerate(words):
                        # Check if this is a single digit followed by a stop word (like "1 -")
                        is_product_number = (re.match(r'^\d$', word) and 
                                           i + 1 < len(words) and 
                                           words[i + 1] in ['-', 'pack', 'tube', 'case', 'pail', 'drum'])
                        
                        # Check if this is a product grade/specification (like 46XC, 10W30, etc.)
                        is_product_grade = (re.match(r'^\d+[A-Z]{1,3}$', word) or  # Like 46XC, 10W, 5W30
                                          re.match(r'^[A-Z]+\d+[A-Z]*$', word))    # Like XC46, W30
                        
                        # Stop at technical specifications (but include single product numbers and grades)
                        if (re.match(r'^\d{3,}$', word) or  # Stop at 3+ digit numbers (not grades like 46)
                            word.lower() in ['pack', 'tube', 'case', 'pail', 'drum', 'gallon', 'liter', 'kg', 'lbs'] or
                            (re.match(r'^\d+[a-z]+$', word.lower()) and not is_product_grade) or  # Numbers with lowercase letters (like 30x400g) but not grades
                            'x' in word.lower() or
                            'g' == word.lower() or
                            (word == '-' and not is_product_number)):
                            
                            # If this is a single digit before a stop word, include it
                            if is_product_number:
                                description_words.append(word)
                            break
                        
                        # Keep meaningful product name words (including single digits)
                    ...
                    if description_words:
                        ...
                        # Add space before numbers (MOV Long Life0 -> MOV Long Life 0)
                        description = re.sub(r'([a-zA-Z])(\d)', r'\1 \2', description)
                        # Clean up extra spaces
                        description = re.sub(r'\s+', ' ', description).strip()
                    else:
                        # Fallback to first few words
                        description = ' '.join(raw_description.split()[:3])
                        description = re.sub(r'([a-zA-Z])(\d)', r'\1 \2', description)
                    
                    # Clean unit - extract just the unit type
                    unit_clean = re.sub(r'\d+', '', unit)  # Remove numbers from unit
                    if not unit_clean:
                        unit_clean = unit
                ...
                # Standard patterns for other items
                # Pattern 1: Standard format with price - MOVLL1P17 15 PAIL MOV Long Life 1 - PAILS 601.43 US$9,021.45
                item_match = re.match(r'^([A-Za-z0-9]+)\s+(\d+)\s+([A-Za-z]+)\s+(.*?)\s+([\d,]+\.?\d*)\s+(?:US\$|CAD\$|\$)?([\d,]+\.?\d*)$', line)
                
                # Pattern 2: Charge format - Pallet 1 Each Pallet charge 35.00 US$35.00
                if not item_match:
                    item_match = re.match(r'^([A-Za-z\s]+?)\s+(\d+)\s+([A-Za-z]+)\s+(.*?)\s+([\d,]+\.?\d*)\s+(?:US\$|CAD\$|\$)?([\d,]+\.?\d*)$', line)
                
                # Pattern 3: Simple charge format - Freight Charge 1 Each Prepaid & Add Shipment
                if not item_match:
                    item_match = re.match(r'^([A-Za-z\s]+?)\s+(\d+)\s+([A-Za-z]+)\s+(.*?)$', line)
                
                if item_match:
                    # Handle different match group counts
                    groups = item_match.groups()
                    item_code = groups[0].strip()
                    quantity = int(groups[1])
                    unit = groups[2]
                    
                    # Clean up description - extract just the product name
                    raw_description = groups[3].strip() if len(groups) > 3 else ''
                    
                    # Generic description cleaning for any product (same logic as merged lines)
                    description_words = []
                    words = raw_description.split()
                    
                    # Find the core product name
                    for i, word in enumerate(words):
                        # Check if this is a single digit followed by a stop word (like "1 -")
                        is_product_number = (re.match(r'^\d$', word) and 
                                           i + 1 < len(words) and 
                    ...
                    # Final cleanup
                    description = description.rstrip(' H').rstrip(' G').rstrip(' T').strip()
                    ...
                    item = {
                        ...
                        'unit': unit,
                        'description': description,
                        'unit_price': safe_float(groups[4]) if len(groups) > 4 and groups[4] else 0.0,
                        'total_price': safe_float(groups[5]) if len(groups) > 5 and groups[5] else 0.0,
                        'raw_line': line
                    }
                    items.append(item)
                else:
                    # Only add as unparsed item if it looks like a product line
                    # Must have quantity and unit keywords
                    if re.search(r'\b\d+\s+(DRUM|PAIL|CASE|EACH|GALLON|KG|L)\b', line, re.IGNORECASE):
                        # This might be an item in a different format
                        parts = line.split()
        ...
        if not items:
            for line in lines:
                line = line.strip()
                # Look for lines that are definitely product entries (not instructions)
                # Must have: product code, quantity, unit, and NOT contain instruction keywords
                if (re.search(r'^[A-Z0-9-]+\s+\d+\s+(DRUM|PAIL|CASE|EACH|GALLON|KG|L|TUBE)', line, re.IGNORECASE) and
                    not any(keyword in line.upper() for keyword in ['EMAIL', 'INVOICE', 'BOL', 'FREIGHT', 'ESTES', 'ACCOUNT', 'SHIPPED', 'COMMENT'])):
                    items.append({
                        'description': line,
                        'raw_line': line,
                        'parsed': False
        ...
        # Determine status from directory path (more reliable)
